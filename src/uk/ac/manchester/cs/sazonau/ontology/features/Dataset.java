...
import uk.ac.manchester.cs.sazonau.engine.PerformanceTester;
...
public class Dataset extends ArrayList<DatasetEntry>{
	...
	public void addTrivialEntry() {
		DatasetEntry en = new DatasetEntry();
		en.time = 0;
		en.vector = new OntFeatureVector().toArray();
		this.add(en);
	}
	...
	public Dataset createSizeDataset() {
		Dataset set = new Dataset();
		for (DatasetEntry en : this) {
			DatasetEntry sizeEn = new DatasetEntry();
			sizeEn.id = en.id;
			sizeEn.time = en.time;
			sizeEn.vector = new Double[1];
			sizeEn.vector[0] = en.countAxioms();
			set.add(sizeEn);
		}		
		return set;
	}
	...
	public void repairTimeout() {
		if (isAllTimeout()) {
			for (DatasetEntry en : this) {
				en.time = PerformanceTester.TIMEHALT*1.5;
			}
		}
	}
	
	private boolean isAllTimeout() {		
		for (DatasetEntry en : this) {
			if (en.time < PerformanceTester.TIMEOUT*PerformanceTester.TFRAC) {
				return false;
			}
		}
		return true;
